...
- Executes the resolved query via a pluggable executor (`internal/executors`): `postgres` (`jackc/pgx`) and `lake` (DuckDB via `go-duckdb`, scanning Delta tables on RustFS — CGO, hence broker's non-default `CGO_ENABLED=1` build). A single-source Projection resolves to one executor call; a multi-source Projection (routed sources, see below) queries every source in order, stitching pages that straddle a routing boundary so pagination looks seamless regardless of how many sources answer it.
...
The reason `Sync` + `Projection` + `broker` exist as separate concepts: callers query a named `Projection` and never know or care which physical store/connection/table currently answers it. `QueryPlanner.ResolvePlan` (`core/proto/controlplane/v1/query_plan.proto`) is the resolution seam; it returns one `StorePlan` per Projection source, ordered youngest to oldest, letting `broker` route (and stitch) pages across any number of physical stores without the caller ever knowing.

**Per-source routing and view**: each entry in a Projection's `spec.sources` carries its own `view` (`table`/path, and optionally a renamed/subset `columns` list — passthrough otherwise) and, when the Projection has more than one source, its own `routing` (a discriminated union; only `type: timeRange` is implemented, carrying `timestampColumn` + a `minAge`/`maxAge` window). Routing windows across a Projection's sources must partition time exactly — no gaps or overlaps, validated at apply time. This is a purely Projection-level, query-time routing decision — it's independent of a Sync's own `spec.retention` (an ingest-time concern: how long `workerd` physically prunes a postgres target), so a source with no retention concept at all still routes correctly. See `examples/tiered-events/` for a full worked example (`events-to-postgres` routed to rows younger than 168h, `events-to-lake` routed to 168h and older, `events-combined` spanning both).
