...
import { buildProjectionSpec, parseYamlDocument } from "@/lib/resourceForm";
...
export default function ProjectionEditor({
  ...
  const [yamlState, yamlAction, yamlPending] = useActionState(
    applyRawResourceAction.bind(null, "Projection", mode === "edit" ? initial?.name : undefined),
  ...
  function switchToYaml() {
    ...
    const spec = buildProjectionSpec(fd);
    setYamlText(
      stringify({ apiVersion: "dal.io/v1alpha1", kind: "Projection", metadata: { name }, spec }),
    );
  ...
  function switchToForm() {
    ...
    if (doc.kind !== "Projection") {
      setYamlError('kind must be "Projection"');
      return;
    }
    setCurrentInitial({
      kind: "Projection",
      name: doc.name,
      spec: doc.spec as ProjectionResource["spec"],
      raw: yamlText,
    });
    setFormKey((k) => k + 1);
    setYamlError(undefined);
